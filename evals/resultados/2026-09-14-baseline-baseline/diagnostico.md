...
|---|---|
| run_id | `267100f99eed` |
| fecha | 2026-09-14T18:50:16+00:00 |
| prompt / reglas / esquema | `baseline` / `baseline` / `v1.1` |
| modelo | `replay` |
| commit | `343efe3-sucio` |
| corridas por eval | 3 |
| llamadas (desde caché) | 15 (0) |
...
| ¿Faltó contexto? | **NO** | no hay fallos consistentes en todas las corridas _(derivada de «¿Falló el prompt?», no es una señal independiente)_ |
| ¿El schema permite algo incorrecto? | **NO** | el output respetó forma, enums y rangos del esquema congelado |
| ¿La tool devolvió mal? | **NO** | JSON válido y completo en 3/3 corridas, sin truncamiento. Hubo 1 reintento(s), que no cuentan como fallo. |
| ¿Faltó validación? | **NO** | todo lo que falló en crudo fue corregido por el código, o no hubo fallos |
| ¿Elegimos mal el modelo? | **SIN DATO** | 3 corridas no bastan para separar un flake de un defecto (hacen falta 5). Sube n: con caché es barato. |
...
| ¿Faltó contexto? | **NO** | no hay fallos consistentes en todas las corridas _(derivada de «¿Falló el prompt?», no es una señal independiente)_ |
| ¿El schema permite algo incorrecto? | **NO** | el output respetó forma, enums y rangos del esquema congelado |
| ¿La tool devolvió mal? | **NO** | JSON válido y completo en 3/3 corridas, sin truncamiento. |
| ¿Faltó validación? | **NO** | todo lo que falló en crudo fue corregido por el código, o no hubo fallos |
| ¿Elegimos mal el modelo? | **SIN DATO** | 3 corridas no bastan para separar un flake de un defecto (hacen falta 5). Sube n: con caché es barato. |
...
| ¿Faltó contexto? | **NO** | no hay fallos consistentes en todas las corridas _(derivada de «¿Falló el prompt?», no es una señal independiente)_ |
| ¿El schema permite algo incorrecto? | **NO** | el output respetó forma, enums y rangos del esquema congelado |
| ¿La tool devolvió mal? | **NO** | JSON válido y completo en 3/3 corridas, sin truncamiento. Hubo 4 reintento(s), que no cuentan como fallo. |
| ¿Faltó validación? | **NO** | todo lo que falló en crudo fue corregido por el código, o no hubo fallos |
| ¿Elegimos mal el modelo? | **SIN DATO** | 3 corridas no bastan para separar un flake de un defecto (hacen falta 5). Sube n: con caché es barato. |
...
| ¿El schema permite algo incorrecto? | **NO** | el output respetó forma, enums y rangos del esquema congelado |
| ¿La tool devolvió mal? | **NO** | JSON válido y completo en 3/3 corridas, sin truncamiento. |
| ¿Faltó validación? | **SÍ** | fallaron en crudo Y siguieron fallando tras el validador: ['hitl_marcado', 'version_correcta'] |
| ¿Elegimos mal el modelo? | **SIN DATO** | 3 corridas no bastan para separar un flake de un defecto (hacen falta 5). Sube n: con caché es barato. |
| ¿Requiere human-in-the-loop? | **SÍ** | sí, y el modelo NO la marcó solo (['hitl_marcado']): la forzó el código |
